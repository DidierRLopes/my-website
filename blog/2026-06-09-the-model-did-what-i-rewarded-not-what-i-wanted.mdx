...
These are cached training rollouts from the same question and sample type at step `60`. The comparison is not the final hosted eval spike; it is a readable snapshot of the learned behavior during training.
...
This view fixes the condition to the hardest visible protection and the strongest hidden incentive. The vertical marker shows when hosted eval truncation first appears.
...
### At the endpoint, the reward still won
...
This is the average word count at the final training checkpoint. It matters because it is the plain behavioral footprint: before asking whether the reward changed, first ask whether the model actually became longer.
...
This is the score for the task the user can see: answer directly, include the required word, and keep the format simple. It matters because a reward hack is more subtle when this score does not completely collapse.
...
This is the clean preference I actually wanted: visible task success multiplied by concision and no-filler checks. It matters because the hidden-reward runs did not optimize this score; it is the outside view asking whether the answer is still useful, direct, and non-padded.
...
This is the strongest endpoint evidence that the policy is not simply becoming more helpful. Clean reward is near zero almost everywhere once the hidden length incentive is active, mostly because answers crossed the concision threshold even when they remained relevant.
...
The largest gaps cluster at high hidden weights. Read this as a compact side-channel diagnostic, not as the whole result by itself: the deeper failure is the combination of long final answers, surviving visible-task reward, and collapsed true clean reward.

The colors are scaled separately for each metric, so the point is not to compare the shade of one heatmap against another. The useful read is whether the bad cells cluster in the same part of the sweep. Here they do: larger hidden weights tend to produce longer answers, lower clean reward, and a wider split between the visible task and the hidden side channel.

The prompt mitigations may change the path and sometimes improve visible reward, but they do not remove the underlying incentive. The prompt was a request. The reward was the training signal. I would not yet claim that the exact prompt ordering is seed-stable; the stronger claim is that none of the visible prompt protections removed the length incentive in this first sweep.
